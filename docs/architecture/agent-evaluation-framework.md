...
当原始目标承诺“全面”“全部代码”“深度审计”或“S 级交付”时，评估器必须单独输出“审计方法充分性”结论，不能只在模块覆盖或误报章节中顺带提及。

- 正则、`grep`、关键词、secret scan、依赖扫描和其他固定模式匹配只能发现候选线索、辅助定位代码，不能替代语义代码审计，也不能单独证明漏洞不存在或代码已被完整审计。
- 合格方法必须为每个仓库建立完整代码/模块清单，并逐模块检查入口与调用链、外部输入与数据流、信任边界、认证授权、敏感操作、配置和依赖；结合语言感知的 AST/语义分析、人工式代码推理及必要的非破坏性验证。
- 每个重要模块和每个适用漏洞类别都必须有“已检查范围、方法、原始证据、发现或无发现结论、局限”的覆盖账本。文件被扫描、报告非空或规则命中均不能替代该账本。
- 若主要工作是统一正则批扫、模板化报告、仅对命中位置阅读，或只对少数高风险样本深审，则方法充分性必须判 `FAIL`；报告必须直言该任务完成的是“模式扫描/初筛”，不是“全面代码审计”。
- 抽样是评估器验证任务质量的手段，不是执行者满足全量目标的手段。评估器可以通过分层抽样发现方法缺陷，但不得以“抽样未发现问题”推断全部目标合格。
- 目标过多、并发或预算不足、要求全自动化，都不是降低原始目标深度的合理理由。它们应被记录为拆分、资源配置或承诺可行性缺陷；若无法在约束内完成，应诚实缩小承诺或判未完成。

报告至少回答：实际方法属于模式扫描、语义审计还是混合方法；模式工具承担何种角色；是否逐仓库、逐模块阅读与追踪；调用链、数据流、信任边界和漏洞类别覆盖率；该方法能证明及不能证明什么；达到原目标还需补做哪些全量工作。
...
评估器必须使用章节化提交协议，普通 Assistant 文本仅用于实时过程展示，不能被系统认定为完成：

1. 调查完一个章节后调用 `task_audit_submit_section`，使用固定 `sectionId` 提交或覆盖该章。工具逐章校验最低内容长度和该章必须包含的判断维度；成功后只终止当前模型 run，由审计编排器保留会话状态并定向提示下一缺失章节，避免模型先无限调查、最后才集中撰写。
2. 11 个必需章节全部提交后调用 `task_audit_generate_report`，同时提交任务质量分、平台健康度、等级、置信度、六项硬门禁和五层覆盖结论。
3. 最终工具不得接受模型直接传入整篇报告；它按固定目录和顺序从已校验章节确定性组装 Markdown。缺少章节时必须返回全部缺失章节的 ID 与名称，保持 Agent loop 继续，由评估器补交后重试。
4. 只有最终工具成功才终止 Agent loop。模型提前停止时，应在保留既有证据上下文和已提交章节的同一会话中继续，不能接受过渡语或从空上下文重跑。

固定章节依次为：审计结论与任务结果质量、Issue 拆分质量、Agent 执行与协作效率、关键成果与形成线路、工具使用与工具优化、Agent 与模型问题、Agent 系统与协作机制、其他问题与遗漏风险、关键时间线/首次失效点与根因、整改优先级与复测方案、证据局限与未能证明事项。

| 顺序 | `sectionId` | 章节 | 必须回答的核心问题 |
| --- | --- | --- | --- |
| 1 | `conclusion_and_result` | 审计结论与任务结果质量 | 成功与否、Requirement ledger、目标覆盖矩阵、硬门禁依据、完美度差距、交付质量和方法充分性；代码任务必须区分正则模式扫描与逐模块语义审计以及五层覆盖。 |
| 2 | `issue_decomposition` | Issue 拆分质量 | 拆分前理解是否充分；子任务是否有区分度、无重复遗漏；阶段、依赖、关键路径、粒度、预算与角色是否合理；错误拆分造成何种低效；建议 Issue 树是什么。 |
| 3 | `agent_execution` | Agent 执行与协作效率 | 每个 Agent/Execution 的有效工作、重复、无效探索、等待、重试、返工、预算和恢复；并行是否充分或冲突；信息是否送达、理解并产生行动。 |
| 4 | `key_outcomes` | 关键成果与形成线路 | 所有成果、发现、纠错和可复用资产；每项成果对应的思路线路、Issue、Agent、Execution、工具、关键事件、转折证据和最终贡献；哪些成果未被整合。 |
| 5 | `tooling` | 工具使用与工具优化 | 工具选择、参数、成功率、延迟、重复调用和数据搬运是否高效；新增/改造工具的输入、输出、边界、替代调用链、收益、风险和复测指标。 |
| 6 | `agent_and_model` | Agent 与模型问题 | 具体 Agent、轮次或步骤中的误解、绕路、过早结束、重复、幻觉、上下文丢失和工具误选；根因属于模型、Prompt、上下文、Skill、工具反馈、角色还是预算。 |
| 7 | `system_collaboration` | Agent 系统与协作机制 | 协作模式、状态机、调度并发、预算唤醒、恢复、父子等待、验收、能力装配、Phone/Board、会话、容器、存储和可观测性的系统性问题及优化。 |
| 8 | `other_findings` | 其他问题与遗漏风险 | 以上分类之外的授权、真实性、状态、范围、证据、交付冲突、复现性和用户体验风险；没有额外发现时说明实际检查范围。 |
| 9 | `timeline_and_root_causes` | 关键时间线、首次失效点与根因 | 关键事件顺序与首次失效点；每项缺陷的严重度、reason code、预期/实际、影响、主根因和促成因素；任务问题和平台问题必须分开。 |
| 10 | `remediation` | 整改优先级与复测方案 | 短期 Prompt/规则、中期契约/工具、长期架构的优先级、责任范围、收益和可执行复测；不得通过抽样或弱化目标冒充完成。 |
| 11 | `evidence_limits` | 证据局限与未能证明事项 | 缺失证据、采样边界、冲突、推断、置信度、PASS/FAIL/UNPROVEN，以及改变结论所需的补充证据。 |
