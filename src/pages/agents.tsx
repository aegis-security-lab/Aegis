...
function editAgent(agent: AgentDefinition): SaveAgentInput {
  return {
    ...
    model: {
      ...agent.model,
      pricing: agent.model.pricing ? { ...agent.model.pricing } : null,
    },
    tools: [...agent.tools],
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Metric
          icon={Bot}
          label="可调度类型"
          value={agents.filter((item) => item.enabled).length}
        />
        <Metric
          icon={BrainCircuit}
          label="Skill 绑定"
          value={agents.reduce((sum, item) => sum + item.skillIds.length, 0)}
        />
        <Metric
          icon={Database}
          label="知识库绑定"
          value={agents.reduce(
            (sum, item) => sum + item.knowledgeBaseIds.length,
            0
          )}
        />
      </div>
        ...
        <CardContent className="p-0">
          ...
          {agents.map((agent) => (
            <AgentRow
              ...
              onDelete={async () => {
                if (!window.confirm(`删除 Agent 类型「${agent.name}」？`))
                  return
                try {
                  await deleteAgent(agent.id)
                  await refresh()
                  toast.success("Agent 类型已删除")
                } catch (error) {
                  toast.error(
...
function AgentRow({
  ...
  const capabilityCount =
    2 + agent.skillIds.length + agent.knowledgeBaseIds.length
  return (
    <div className="grid gap-4 border-b px-5 py-4 last:border-b-0 lg:grid-cols-[minmax(12rem,1.2fr)_minmax(14rem,2fr)_8rem_8rem_6rem] lg:items-center">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`size-2 rounded-full ${agent.enabled ? "bg-success" : "bg-muted-foreground/40"}`}
          />
        </div>
      ...
      <div className="min-w-0">
        <p className="line-clamp-2 text-sm text-muted-foreground">
          {agent.description || "未填写职责边界"}
        </p>
        <div className="mt-2 flex flex-wrap gap-1.5">
          <Badge variant="outline">{agent.category}</Badge>
          <Badge variant="secondary">{capabilityCount} 项能力</Badge>
          {agent.skillIds.slice(0, 2).map((id) => (
      ...
      <div className="text-sm">
        <div className="font-medium">{agent.model.model || "继承默认"}</div>
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span>{agent.permissions.approvalMode || "继承"}</span>
      </div>
...
function AgentEditor({
  ...
  agent: AgentDefinition | "new" | null
  onClose: () => void
  onSaved: () => Promise<void>
}) {
  const { state } = useAppState()
  const [form, setForm] = React.useState<SaveAgentInput>(() =>
    agent === "new" || !agent ? blankAgent() : editAgent(agent)
  )
  ...
  const toggleItem = (
    key: "tools" | "skillIds" | "knowledgeBaseIds",
    value: string,
    checked: boolean
  ) =>
    set(
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.systemPrompt.trim()) {
      toast.error("请填写 Agent 类型名称和系统提示词")
      return
    }
    setSaving(true)
    try {
      ...
      <DialogContent className="max-h-[88vh] max-w-3xl overflow-hidden p-0">
        <form onSubmit={submit} className="flex max-h-[88vh] flex-col">
          <DialogHeader className="border-b px-6 py-5">
            <DialogTitle>
              {agent === "new" ? "新建 Agent 类型" : `编辑 ${agent.name}`}
            </DialogTitle>
            <DialogDescription>
              定义角色职责、模型、Skills、知识和权限。Coordination 与任务 Phone
              由运行时自动提供；个人名字和手机不在这里配置。
            </DialogDescription>
          </DialogHeader>
          <ScrollArea className="min-h-0 flex-1">
            <div className="p-6">
              <Tabs defaultValue="definition">
                <TabsList>
                  <TabsTrigger value="definition">角色定义</TabsTrigger>
                  <TabsTrigger value="capabilities">能力</TabsTrigger>
                  <TabsTrigger value="runtime">运行权限</TabsTrigger>
                </TabsList>
                <TabsContent value="definition" className="pt-5">
                  <FieldGroup>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <Field>
                        <FieldLabel htmlFor="agent-name">类型名称</FieldLabel>
                        <Input
                          id="agent-name"
                          value={form.name}
                          onChange={(event) => set("name", event.target.value)}
                          placeholder="例如：红队工程师"
                        />
                      </Field>
                      <Field>
                    ...
                    <Field>
                      ...
                      <Textarea
                        ...
                        placeholder="说明适合接收什么任务、产出什么证据。"
                      />
                    </Field>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <Field>
                        <FieldLabel htmlFor="agent-category">分类</FieldLabel>
                        <Input
                          id="agent-category"
                          value={form.category}
                          onChange={(event) =>
                    ...
                    <Field>
                      <FieldLabel htmlFor="agent-prompt">系统提示词</FieldLabel>
                      <Textarea
                        id="agent-prompt"
                ...
                <TabsContent value="capabilities" className="space-y-6 pt-5">
                  <div className="rounded-md border bg-muted/25 p-4 text-sm">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      <Badge variant="secondary">phone/default</Badge>
                      <Badge variant="secondary">phone_board_*</Badge>
                      <Badge variant="secondary">phone_relay_*</Badge>
                    </div>
                  </div>
                  <CapabilityGroup
                    title="Skills"
                    icon={BrainCircuit}
                    items={(state?.skills ?? []).map((item) => item.id)}
                    selected={form.skillIds}
                    onToggle={(id, checked) =>
                      toggleItem("skillIds", id, checked)
                    }
                  />
                  <CapabilityGroup
                    title="知识库"
                    icon={Database}
                    items={(state?.knowledgeBases ?? []).map((item) => item.id)}
                    selected={form.knowledgeBaseIds}
                    onToggle={(id, checked) =>
                      toggleItem("knowledgeBaseIds", id, checked)
                    }
                  />
                </TabsContent>
                        ...
                        onValueChange={(value) =>
                          set("permissions", {
                            ...
                            approvalMode:
                              value === "inherit"
                                ? ""
                                : (value as PermissionBoundary["approvalMode"]),
                          })
                        }
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="inherit">继承系统</SelectItem>
                          <SelectItem value="none">无需审批</SelectItem>
                          <SelectItem value="risky">仅高风险</SelectItem>
          ...
          <DialogFooter className="border-t px-6 py-4">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? <Spinner data-icon="inline-start" /> : null}保存类型
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {items.map((item) => (
          ...
          >
            ...
            />
            <code className="truncate text-xs">{item}</code>
          </label>
        ))}
      </div>
      {items.length === 0 ? (
        <p className="text-sm text-muted-foreground">暂无可绑定项</p>
      ) : null}
    </section>
  )
}
