...
func TestGoalPreservingDecompositionPromptTargetsOnlyCapableLeaders(t *testing.T) {
	...
	for _, required := range []string{
		...
		"treat every target as a separate goal",
		"You do not need to create every target Issue up front",
		"complete, auditable coverage ledger",
		"not_created, todo, in_progress, in_review, done, failed, and cancelled",
		"must receive its own execution Issue before any substantive work begins",
		"must never perform several targets as one execution",
		`"Successive waves" means delaying creation or assignment`,
